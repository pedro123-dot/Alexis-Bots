/* URL: //https://repl.it/@brusce/Alexis-Bots-Free
//Copiar-----> wss://Alexis-Bots-Free.brusce.repl.co

[AA]Discord Clan:https://discord.gg/ym7zrw
Discord:[AA]Alexis_X#5882
𝙎𝙞 𝙏𝙞𝙚𝙣𝙚𝙨 𝘼𝙡𝙜𝙪𝙣 𝙋𝙧𝙤𝙗𝙡𝙚𝙢𝙖 𝙃𝙖𝙗𝙡𝙖𝙢𝙚 𝙖 𝙈𝙞
𝘿𝙞𝙨𝙘𝙤𝙧𝙙 :) 𝙔𝙤 𝙏𝙚 𝙖𝙮𝙪𝙙𝙖𝙧𝙚 𝙊 𝙨𝙞 𝙉𝙤 𝙚𝙨𝙩𝙤𝙮 
𝘾𝙤𝙣𝙚𝙘𝙩𝙖𝙙𝙤 𝙃𝙖𝙗𝙡𝙖 𝘾𝙤𝙣 𝙐𝙣 𝙈𝙤𝙙 𝙊 𝘼𝙙𝙢𝙞𝙣 𝙋𝙖𝙧𝙖
𝙌𝙪𝙚 𝙏𝙚 𝙋𝙪𝙚𝙙𝙖𝙣 𝘼𝙮𝙪𝙙𝙖𝙧 𝘾𝙤𝙣 𝙇𝙤𝙨 𝘽𝙤𝙩𝙨 
𝙍𝙚𝙘𝙪𝙚𝙧𝙙𝙖 𝙈𝙞𝙧𝙖𝙧 𝘽𝙞𝙚𝙣 𝙀𝙡 𝙑𝙞𝙙𝙚𝙤 𝙎𝙞𝙜𝙪𝙚 
𝙇𝙤𝙨 𝙥𝙖𝙨𝙤𝙨 𝙔 𝙏𝙚 𝙁𝙪𝙣𝙘𝙞𝙤𝙣𝙖𝙧𝙖 :)

Canal YT Bots Free ᶜᵒᵖʸ: https://www.youtube.com/watch?v=5H1iRkhIutk
*/
...
const WebSocket = require('ws'),
    ...
    facebookHandler = require("./core/facebookHandler.js"),
    proxyManager = require('./core/proxies.js'),
...
if (config.server.update) {
    requester(config.server.link, (err, req, data) => {
        ...
        if (config.server.version < requesterConfig.server.version) {
            logger.warn(`[SERVER] Se encontró una nueva actualización.!`)
            logger.warn('[SERVER] Descargar ->' + config.server.download)
            process.exit()
        } else {
    ...
    logger.error('[SERVER] La actualización es falsa!')
}

logger.good(`[SERVER] Versión en ejecución ${config.server.version} on port ${config.server.port}`);
logger.good(`[PROXY] ${proxies.proxies.length} proxies cargados.
CreaDor [AA]Alexis_X#5882
New Version Bot Free
Agar.io
`);
const game = {
    url: '',
...
const dataBot = {
    ...
    onclose() {
        if (this.isConnected) {
            this.isConnected = false;
            this.connect();
            logger.error('[SERVER] AnalizandoBot Desconectado!')
        }
    },
    handleBuffer(buffer) {
        ...
        switch (reader.readUint8()) {
            ...
            case 241:
                ...
                logger.good('[SERVER] AnalizandoBot Conectado!');
                break
        }
    }
};
